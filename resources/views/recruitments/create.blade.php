<x-app-layout>
    <div class="container lg:w-1/2 md:w-4/5 w-11/12 mx-auto mt-8 px-8 bg-indigo-900 shadow-md rounded-md">
        <h2 class="text-center text-lg text-white font-bold pt-6 tracking-widest">募集情報登録</h2>
        ...
        <form action="{{ route('recruitments.store') }}" method="POST"
            ...
            <div class="mb-4">
                ...
                <input type="text" name="name"
                    ...
                    required placeholder="例: Todoアプリ" value="{{ old('name') }}">
            </div>
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <textarea name="description" rows="4"
                    class="rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-pink-600 w-full py-2 px-3"
                    required placeholder="例: Todoリストを管理するアプリです。">{{ old('description') }}</textarea>
            </div>
            <div class="mb-4">
                ...
                <input type="text" name="period"
                    class="rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-pink-600 w-full py-2 px-3"
                    required placeholder="例: 150~200時間程度" value="{{ old('period') }}">
            </div>
            <div class="mb-4">
                ...
                <input type="text" name="number"
                    class="rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-pink-600 w-full py-2 px-3"
                    required placeholder="例: 3人程度" value="{{ old('number') }}">
            </div>
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <textarea name="gain" rows="4"
                    class="rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-pink-600 w-full py-2 px-3"
                    required placeholder="例: 業務知識、多人数開発の楽しさと難しさ、gitの練習">{{ old('gain') }}</textarea>
            </div>
            <div class="mb-4">
                ...
                <textarea name="caution" rows="4"
                    class="rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-pink-600 w-full py-2 px-3"
                    required placeholder="例: 週に10時間は開発できる時間を確保してください。経験、未経験は問いません。">{{ old('caution') }}</textarea>
            </div>
            <div class="mb-4">
                ...
                <textarea name="comment" rows="4"
                    class="rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-pink-600 w-full py-2 px-3"
                    required placeholder="例: 一緒に楽しく学びましょう！">{{ old('comment') }}</textarea>
            </div>
